/* Kanzu — Kanban (Zoom + Multi-boards + GitHub Gists)
 * - Per-board zoom via --zoom wrapper scale
 * - Local boards + Gist boards
 * - GitHub Personal Access Token (gist scope) flow, all client-side
 * - Create/Update Gists, list & open .kanban.json files
 * - Debounced autosync if board is linked to a Gist; periodic auto-pull
 */
...
function slugify(s){ return (s||"kanban").toLowerCase().replace(/[^\w\-]+/g,"-"); }
function escapeHTML(s){ return (s||"").replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])); }

/////////////////////// Keys & State ///////////////////////
const INDEX_KEY  = "kanzu.index.v1";    // [{id,title,updated,source,gistId?,gistFile?}]
const BOARD_PREFIX = "kanzu.board.";    // per-board storage key
const UNDO_KEY   = "kanzu.undo";
const THEME_KEY  = "kanzu.theme";
const GIST_TOKEN_KEY = "kanzu.github.token";

let boardsIndex = [];     // local index
let board = null;         // current board obj
let currentBoardId = null;
let autoGitTimer = null;  // setInterval id

// GitHub API state
let githubToken = null;
let githubUser = null;
let gistBoardsCache = []; // [{id, filename, updated_at, description}]
...
/////////////////////// Index & Local Save ///////////////////////
function loadIndex(){
  try { boardsIndex = JSON.parse(localStorage.getItem(INDEX_KEY)) || []; }
  catch { boardsIndex = []; }
}
...
function saveBoardLocal(b){
  ...
  const meta = {
    id: b.id, title: b.title, updated: Date.now(),
    source: b.meta?.gistId ? "github" : "local",
    gistId: b.meta?.gistId || null,
    gistFile: b.meta?.gistFile || null
  };
  if (i >= 0) boardsIndex[i] = { ...boardsIndex[i], ...meta };
  else boardsIndex.push(meta);
  saveIndex();
}

function loadBoardLocal(id){
...
function migrateFromSingleBoardIfNeeded(){
  ...
  if (!legacy) return;
  let oldBoard = null; try { oldBoard = JSON.parse(legacy); } catch {}
  if (!oldBoard) return;
  const nb = defaultBoard();
  nb.columns = oldBoard.columns || nb.columns;
  nb.title = (localStorage.getItem("kanzu.title") || "My Kanban");
  nb.settings.theme = (localStorage.getItem(THEME_KEY) || "dark");
  saveBoardLocal(nb);
  localStorage.removeItem("kanzu.board.v1");
  localStorage.removeItem("kanzu.title");
...
function render(){
  ...
  bindTitle();
  applyTheme(board.settings?.theme || "dark");
  applyZoom(board.settings?.zoom ?? 1);
  ...
  for (const col of board.columns){
    ...
    $(".delete-col", menu).addEventListener("click", () => {
      ...
      render();
      scheduleGitSync("delete column");
      toast("Column deleted — Undo available");
    });
    ...
    enableColumnDrag(node);
    enableCardDrop(cardsEl, col.id);
...
function renderCard(col, card){
  ...
  if (due){
    ...
    dueEl.textContent = label + ` • ${due.toISOString().slice(0,10)}`;
    dueEl.classList.add(cls);
...
function addColumn(name="New Column"){
  ...
  board.columns.push({ id: uid(), name, order: nextOrder, cards: [] });
  render();
  scheduleGitSync("add column");
  toast("Column added");
}
function addCard(columnId, data){
  const col = board.columns.find(c => c.id === columnId); if (!col) return;
  const newCard = {
    id: uid(), title: (data.title||"Untitled").trim(),
    desc: (data.desc||"").trim(), color: data.color||"#6aa5ff",
    due: data.due || null, created: Date.now(), updated: Date.now()
  };
  col.cards.unshift(newCard);
  saveBoardLocal(board);
  render();
  scheduleGitSync("add card");
  toast("Card added");
  return newCard.id;
}
...
function deleteCard(columnId, cardId){
  ...
  saveBoardLocal(board);
  render();
  scheduleGitSync("delete card");
  toast("Card deleted — Undo available");
}
...
function enableColumnDrag(colEl){
  colEl.addEventListener("dragstart", (e) => {
    ...
    e.dataTransfer.setData("text/column-id", colEl.dataset.columnId);
    colEl.setAttribute("aria-grabbed", "true");
  });
  colEl.addEventListener("dragend", () => colEl.removeAttribute("aria-grabbed"));
  colEl.addEventListener("dragover", (e) => {
    e.preventDefault();
    const draggingColumnId = e.dataTransfer.getData("text/column-id");
    if (draggingColumnId) colEl.classList.add("drag-over");
  });
  colEl.addEventListener("dragleave", () => colEl.classList.remove("drag-over"));
  colEl.addEventListener("drop", (e) => {
    e.preventDefault(); colEl.classList.remove("drag-over");
    const draggingColumnId = e.dataTransfer.getData("text/column-id");
    if (!draggingColumnId || draggingColumnId === colEl.dataset.columnId) return;
    const fromIdx = board.columns.findIndex(c => c.id === draggingColumnId);
    const toIdx = board.columns.findIndex(c => c.id === colEl.dataset.columnId);
    if (fromIdx < 0 || toIdx < 0) return;
    const [moved] = board.columns.splice(fromIdx, 1);
    board.columns.splice(toIdx, 0, moved);
    renumberColumnOrders();
    render();
    scheduleGitSync("reorder columns");
...
function enableCardDrag(cardEl){
  cardEl.addEventListener("dragstart", (e) => {
    ...
    const srcCol = cardEl.closest(".column").dataset.columnId;
    e.dataTransfer.setData("text/src-col-id", srcCol);
  });
...
function enableCardDrop(cardsEl, toColumnId){
  cardsEl.addEventListener("dragover", (e) => {
    ...
    e.preventDefault();
    if (hasCard){
      const afterEl = getCardAfter(cardsEl, e.clientY);
  ...
  cardsEl.addEventListener("drop", (e) => {
    ...
    if (srcColId === toColumnId){
      ...
      saveBoardLocal(board); scheduleGitSync("reorder cards");
    ...
    saveBoardLocal(board); render(); scheduleGitSync("move card");
    toast("Card moved");
  });
}
...
function openCardModal({ columnId, cardId }){
  const isNew = !cardId;
  $("#modal-title").textContent = isNew ? "Add Card" : "Edit Card";
  $("#delete-card").style.display = isNew ? "none" : "";
  fields.columnId.value = columnId;
  fields.id.value = cardId || "";
  if (isNew){
    fields.title.value = ""; fields.desc.value = ""; fields.color.value = "#ffd166"; fields.due.value = "";
  } else {
    const col = board.columns.find(c => c.id === columnId);
    const card = col.cards.find(c => c.id === cardId);
    fields.title.value = card.title; fields.desc.value = card.desc || "";
    fields.color.value = /^#[0-9a-f]{6}$/i.test(card.color) ? card.color : "#6aa5ff";
    fields.due.value = card.due || "";
  }
  modal.showModal();
  fields.title.focus();
}
...
window.addEventListener("keydown", (e) => {
  if (e.key.toLowerCase() === "n" && !modal.open){
    const firstCol = board?.columns?.[0]?.id; if (firstCol) openCardModal({ columnId: firstCol });
  }
});
...
$("#new-board").addEventListener("click", () => {
  const b = defaultBoard(); saveBoardLocal(b); loadBoardById(b.id);
  boardsModal.close(); toast("New board created");
});
$("#dup-board").addEventListener("click", () => {
  ...
  saveBoardLocal(nb); loadBoardById(nb.id);
  boardsModal.close(); toast("Duplicated board");
});
$("#del-board").addEventListener("click", () => {
  ...
  boardsIndex = boardsIndex.filter(x => x.id !== board.id);
  saveIndex();
...
function refreshBoardsLists(){
  // Local
  localList.innerHTML = boardsIndex
    .map(m => `<li><label><input type="radio" name="pickBoard" value="${m.id}" ${m.id===currentBoardId?'checked':''}/> ${escapeHTML(m.title)}</label><span class="meta">${new Date(m.updated || Date.now()).toLocaleString()}</span></li>`)
    .join("") || `<li class="muted">No local boards yet.</li>`;

  // GitHub
  githubList.innerHTML = githubToken ? "" : `<li class="muted">Connect GitHub to list boards.</li>`;
  ...
  gistBoardsCache.forEach(g => {
    ...
    line.innerHTML = `<label><input type="radio" name="pickBoard" value="${id}"/> ${escapeHTML(g.filename)}</label><span class="meta">${new Date(g.updated_at).toLocaleString()}</span>`;
    githubList.appendChild(line);
  });
}

function openSelectedBoard(){
  const sel = document.querySelector('input[name="pickBoard"]:checked');
  if (!sel) { toast("Pick a board first"); return; }
  const val = sel.value;
  if (val.startsWith("gist-")){
    const [_, rest] = val.split("gist-");
    const [id, filename] = rest.split("::");
    openGistBoard(id, filename);
  } else {
    loadBoardById(val);
  }
  boardsModal.close();
...
$("#export-btn").addEventListener("click", () => {
  saveBlobJSONDownload(`${slugify(board.title)}.kanban.json`, board);
  toast("Exported!");
});
$("#import-file").addEventListener("change", async (e) => {
  ...
  try{
    ...
    if (confirm("Import file as a NEW board? (Cancel = merge into current)")){
      const nb = { ...data, id: uid() };
      if (!nb.settings) nb.settings = { theme: document.body.dataset.theme, zoom: 1 };
      saveBoardLocal(nb); loadBoardById(nb.id);
      toast("Imported as new board");
    } else {
      mergeBoardsIntoCurrent(data);
      toast("Merged into current board");
    }
  } catch { toast("Failed to import: invalid JSON"); }
...
function mergeBoardsIntoCurrent(other){
  for (const inc of other.columns || []){
    ...
    if (existing){
      const sig = c => `${(c.title||"").trim()}|${c.due||""}`;
      const have = new Set(existing.cards.map(sig));
      for (const card of inc.cards || []){
        if (!have.has(sig(card))) existing.cards.push({ ...card, id: uid() });
      }
    } else {
...
$("#theme-toggle").addEventListener("click", () => {
  ...
  applyTheme(board.settings.theme); saveBoardLocal(board); scheduleGitSync("theme toggle");
});
...
$("#reset-btn").addEventListener("click", () => {
  ...
  fresh.id = board.id; // keep same local id
  // preserve gist link if you want autosync to continue; or comment these two lines to break link:
  fresh.meta.gistId = board.meta.gistId;
  fresh.meta.gistFile = board.meta.gistFile;
  fresh.meta.githubSync = board.meta.githubSync;
  board = fresh; saveBoardLocal(board); render(); scheduleGitSync("reset to fresh");
...
function loadGithubToken(){
  ...
  githubToken = t || null;
  setGithubStatus(githubToken ? "GitHub: Connected" : "GitHub: Disconnected");
}
...
async function connectGithub(){
  ...
  const token = prompt(`Paste your GitHub Personal Access Token with the "gist" scope ${hint}:`, existing || "");
  if (token === null) return; // cancelled
  if (!token && existing){ toast("Keeping existing token"); return; }
  if (!token){ toast("No token provided"); return; }
  saveGithubToken(token.trim());
  try{
    const u = await ghGET("/user");
    githubUser = u;
    setGithubStatus(`GitHub: ${u.login}`);
...
async function ghPATCH(path, body){
  ...
  if (!res.ok) throw new Error(`GitHub PATCH ${path} failed: ${res.status}`);
  return res.json();
}
...
async function listGistBoards(){
  ...
  while (page <= 3){ // grab up to ~300 total, usually plenty
    ...
    for (const g of arr){
      ...
      if (f){
        out.push({ id: g.id, filename: f.filename, updated_at: g.updated_at, description: g.description || "" });
      }
    }
    page++;
  }
...
async function openGistBoard(gistId, filename){
  try{
    ...
    if (!file){
      // fallback: first .kanban.json
      file = Object.values(files).find(f => f && f.filename && f.filename.endsWith(".kanban.json"));
    ...
    data.meta = data.meta || {};
    data.meta.gistId = gistId;
    data.meta.gistFile = filename;
    data.meta.githubSync = true;
    saveBoardLocal(data);
    loadBoardById(data.id);
    toast("Opened from GitHub Gist");
  } catch(e){
    console.warn(e);
    toast("Failed to open Gist");
  }
}
...
async function saveCurrentToGithub(createNew=false){
  ...
  try{
    if (createNew || !board.meta?.gistId){
      const body = {
        description: "Kanzu Kanban Board",
        public: false,
        files: { [desiredName]: { content } }
      };
      const res = await ghPOST("/gists", body);
      board.meta.gistId = res.id;
      board.meta.gistFile = desiredName;
      board.meta.githubSync = true;
      board.meta.lastSync = Date.now();
      saveBoardLocal(board);
      await listGistBoards();
      toast("Saved as new Gist");
    } else {
      // Update existing; also rename file if title changed
      const files = {};
      const currentName = board.meta.gistFile || desiredName;
      if (currentName !== desiredName){
        files[currentName] = { filename: desiredName, content };
      } else {
        files[currentName] = { content };
      }
      const res = await ghPATCH(`/gists/${board.meta.gistId}`, { files });
      board.meta.gistFile = desiredName;
      board.meta.githubSync = true;
      board.meta.lastSync = Date.now();
      saveBoardLocal(board);
...
function scheduleGitSync(){
  if (!board?.meta?.githubSync && !board?.meta?.gistId) return;
  if (scheduleGitSync._t) clearTimeout(scheduleGitSync._t);
  scheduleGitSync._t = setTimeout(() => saveCurrentToGithub(false), 1200);
}
...
function loadBoardById(id){
  const b = loadBoardLocal(id);
  if (!b){ toast("Board not found"); return; }
  currentBoardId = id; board = b; render();
}
  ...
  autoGitTimer = setInterval(async () => {
    ...
    try{
      ...
      if (remoteUpdated > localUpdated + 1000){
        // fetch full content (detail already has content)
        if (f.content){
          const data = JSON.parse(f.content);
          if (data?.columns){
            data.id = board.id;
            data.meta = data.meta || {};
            data.meta.gistId = board.meta.gistId;
            data.meta.gistFile = f.filename;
            data.meta.githubSync = true;
            board = data; saveBoardLocal(board); render();
...
function boot(){
  ...
  loadGithubToken();
  if (githubToken){
    // Try to resolve user + list gists, silently
    ghGET("/user").then(u => { githubUser = u; setGithubStatus(`GitHub: ${u.login}`); return listGistBoards(); })
                  .catch(() => { saveGithubToken(null); setGithubStatus("GitHub: Disconnected"); });
  }

  // Load index / migrate / ensure board
  loadIndex(); migrateFromSingleBoardIfNeeded(); loadIndex();

  if (!boardsIndex.length){
    const b = defaultBoard(); saveBoardLocal(b); loadBoardById(b.id);
  } else {
    boardsIndex.sort((a,b) => (b.updated||0) - (a.updated||0));
    loadBoardById(boardsIndex[0].id);
  }
  ...
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape"){
      for (const m of $$(".column-menu")) m.hidden = true;
      if (modal.open) modal.close();
      if ($("#boards-modal").open) $("#boards-modal").close();
    }
  });
